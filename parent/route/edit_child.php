...
<body class="bg-gray-100 overflow-hidden">
    ...
    ?>
        <section class="w-full max-w-[1700px] mx-auto h-full bg-white p-8 overflow-x-hidden overflow-y-auto">
            <h2 class="text-2xl font-bold mb-6 mt-4 text-center text-purple-600">Edit Child Details</h2>
            <div class="w-full max mx-auto bg-white shadow-md rounded-lg p-6">
                ...
                if ($child_id) {
                    $stmt = $conn->prepare("SELECT name, dob, gender FROM children WHERE child_id = ?");
                    ...
                    die('No child_id provided'); 
                }
                ?>
                <form id="editChildForm" action="./update_child.php" method="POST" class="space-y-4">
                    <input type="hidden" id="child_id" name="child_id" value="<?= htmlspecialchars($child_id) ?>">
                    ...
                    <div>
                        ...
                        <input type="date" id="dob" name="dob" required value="<?= htmlspecialchars($child['dob']) ?>"
                            class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-[#66347F] focus:border-[#66347F] sm:text-sm">
                    </div>
                    ...
                    <div>
                        ...
                        <select id="gender" name="gender" required
                            ...
                            <option value="Female" <?= $child['gender'] === 'Female' ? 'selected' : '' ?>>Female
                            </option>
                        </select>
                    </div>
    ...
    $(document).ready(function() {
        $('#editChildForm').on('submit', function(e) {
            ...
            $.ajax({
                url: '../controller/edit_child.controller.php',
                type: 'POST',
                dataType: 'JSON',
                data: formData,
                success: function(response) {
                    if (response.success) {
                        window.location.href = `./children.php?updated_child=${formData.name}&&prev_name=<?= $child['name']?>`;
                        $('#addChildForm')[0].reset();
                    } else {
                        alert('Failed to update child: ' + response.message);
                    }
                },
                error: function() {
                    alert('An error occurred while update the child.');
                },
            });
        });
    });
